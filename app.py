...
if uploaded_file is not None:
    with st.spinner("Procesando..."):
        ...
        # REGEX para tus facturas
        factura = re.search(r'No[\.\s]*\s*([A-Z0-9\-]+)', text, re.I)
        nit = re.search(r'NIT[:\.\s]*([\d\.\-]+)', text, re.I)
        proveedor = re.search(r'^(.+?)(?=NIT|Tel|Direcci)', text, re.M) or re.search(r'(.+?)\n', text)
